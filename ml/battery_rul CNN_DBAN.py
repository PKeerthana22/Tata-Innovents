...
# Load the dataset
df = pd.read_csv('Updated_Battery_RUL_Tiago_Spec.csv')

# Data Visualization
plt.figure(figsize=(12, 6))
sns.histplot(df['RUL (remaining lifecycle)'], kde=True, bins=30)
plt.title('Distribution of RUL (remaining lifecycle)')
plt.xlabel('RUL (remaining lifecycle)')
...
cnn_dban_model = create_cnn_dban_model((8,))
history = cnn_dban_model.fit(X_train, y_train, epochs=3, batch_size=32, validation_data=(X_test, y_test))
cnn_dban_model.save('trained_battery_rul_model.h5')
